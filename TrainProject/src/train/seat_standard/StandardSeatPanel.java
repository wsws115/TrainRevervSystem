...
public class StandardSeatPanel extends JPanel{
...
//   private JFrame frame;
   ...
   JPanel getHalfSeatPanel(String[] positions) {
      ...
      for(int i = 0; i < seatBtns.length; i++) {
         ...
         seatBtns[i].setFont(new Font(mainFont, Font.BOLD, 20));
         seatBtns[i].addActionListener(action);
   ...
   JPanel getseatPanel(JPanel seatMainPanel) {
       ...
       leftSeatPanel.add(getHalfSeatPanel(standardleft), BorderLayout.WEST);
       
       // 우측좌석패널(C,D)
       JPanel rightSeatPanel = new JPanel();
//       rightSeatPanel.setLayout(new BorderLayout());
       rightSeatPanel.setBackground(Color.white);
       rightSeatPanel.add(getHalfSeatPanel(standardright),BorderLayout.EAST);
